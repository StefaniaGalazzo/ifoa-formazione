...
const token =
  "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2NTc0ZDRmYjJjNmEwZDAwMTg0OTYxNjYiLCJpYXQiOjE3MDIxNTU1MTUsImV4cCI6MTcwMzM2NTExNX0.twA9CA1Sr7fFQXqOabyqnlCpbEisUGZ7FrD6uJi_XoY";
const headers = {
  Authorization: token,
    ...
    let newRow = `
            <tr>
                ...
                <td class="justify-content-end">
                    <button type="button" class="btn btn-sm btn-outline-primary mx-1" data-bs-toggle="modal" data-bs-target="#productDetails" onclick="handleModalData('${
                      element._id
                    }','show')">Dettagli</button>
                    <button type="button" class="btn btn-sm btn-outline-warning mx-1" data-bs-toggle="modal" data-bs-target="#productDetails" onclick="handleModalData('${
                      element._id
                    }','edit')">Modifica</button>
                    <button type="button" class="btn btn-sm btn-outline-danger mx-1" onclick="deleteRecord('${
                      element._id
                    }')">Elimina</button>
                </td>
            </tr>
...
async function handleModalData(id, action) {
  ...
  if (id !== null) {
    ...
    document.getElementById("productDetailsTitle").innerText =
      "Dettagli prodotto " + data.name;
    ...
    document.getElementById("productPhoto").alt =
      "Immagine prodotto " + data.name;
    document
      .getElementById("buttonSave")
      .setAttribute("onclick", `sendData('${action}', '${id}')`);
  } else {
    document.getElementById("productDetailsTitle").innerText =
      "Crea nuovo prodotto";
    ...
    document
      .getElementById("buttonSave")
      .setAttribute("onclick", `sendData('${action}', null)`);
  }
}
